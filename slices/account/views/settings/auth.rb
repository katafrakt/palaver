  ...
  def view_template
    ...
    settings_layout(active: :account) do
      render Ui::Components::Form.new(url: "/account/settings/account", multipart: true) do |form|
        ...
        form.field(label: "Email", name: "email", value: @settings.email, disabled: true)

        form.section_title("Password reset", pad_top: true)
        form.field(label: "Current password", name: "current_password", type: :password,
          error: @errors[:current_password], value: @values[:current_password])
        form.field(label: "New password", name: "new_password", type: :password,
          error: @errors[:new_password], value: @values[:new_password])
        form.field(label: "Confirm new password", name: "new_password_confirmation", type: :password,
